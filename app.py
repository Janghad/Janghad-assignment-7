...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    error_item = np.random.normal(mu, np.sqrt(sigma2), N)
    Y = beta0 + beta1 * X + error_item  # Replace with code to generate Y
...
def hypothesis_test():
    ...
    plt.hist(simulated_stats, bins=20, alpha=0.5, color="gray", label=f"{parameter.capitalize()}s")
...
def confidence_interval():
    ...
    plt.axvline(x=true_param, color="orange", linestyle="--", linewidth=2, label="True Parameter")
